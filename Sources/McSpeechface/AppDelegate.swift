...
@MainActor final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    private let service = McSpeechfaceService()
    ...
    private let commandServer = McSpeechfaceCommandSocketServer()
    ...
    private var commandTranscriptionTask: Task<McSpeechfaceCommandResult, Error>?
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let loginItemMigrationError = LoginItemManager.finishLegacyMigration()
        ...
        if let loginItemMigrationError {
            ...
            alert.informativeText = "McSpeechface could not replace the earlier login item. Open Settings > General, turn Launch McSpeechface at login on, and remove any earlier entry in System Settings > General > Login Items. \(loginItemMigrationError.localizedDescription)"
    ...
    private func handleCommand(
        ...
        responder: McSpeechfaceCommandResponder
    ) async {
        do {
            ...
            case .status:
                try await responder.sendSuccess(McSpeechfaceCommandResult(
                    kind: "status",
                    state: commandState,
            ...
            case .correctionModels:
                ...
                let models = TranscriptEditingModel.allCases
                    ...
                    .map { model in
                        ...
                        return McSpeechfaceCommandCorrectionModel(
                            key: model.cliKey,
                            name: model.title,
    ...
    private func handleTranscribeCommand(
        ...
        guard let arguments = request.arguments, let path = arguments.path else {
            throw McSpeechfaceError.message("The command did not include an audio file.")
        }
        let url = URL(fileURLWithPath: path).standardizedFileURL
        guard FileManager.default.isReadableFile(atPath: url.path),
              UTType(filenameExtension: url.pathExtension)?.conforms(to: .audio) == true else {
            throw McSpeechfaceError.message("The requested audio file is unavailable or unsupported.")
        }
        let model: DictationModel
        if let key = arguments.model {
            guard let requested = DictationModel.all.first(where: { $0.key == key }) else {
                throw McSpeechfaceError.message("No McSpeechface model has the key \(key).")
            }
            model = requested
        } else {
    ...
    private func handleRecordStartCommand(
        ...
        do {
            ...
            guard state == .recording else {
                throw McSpeechfaceError.message("McSpeechface could not start recording.")
            }
    ...
    private func resolveCommandCorrection(
        _ requested: McSpeechfaceCommandCorrection?
    ...
    ) async throws -> McSpeechfaceCommandResult {
    ...
    private func matchingCommandRecording(_ request: McSpeechfaceCommandRequest) -> CommandRecording? {
    ...
    private func stopRecording() {
        ...
        do {
            ...
            transcriptionTask = Task { [weak self] in
                ...
                } catch McSpeechfaceError.noSpeechDetected {
                    if self.transcriptionID == transcriptionID {
    ...
    func application(_ application: NSApplication, open urls: [URL]) {
        if urls.count == 1, let url = urls.first, url.scheme == "mcspeechface" {
            guard let section = SettingsSection(deepLink: url) else {
                presentError(McSpeechfaceError.message("McSpeechface could not open that link."))
                return
            }
        ...
        guard UserDefaults.standard.bool(forKey: "setupCompleted") else {
            showSetup()
            presentError(McSpeechfaceError.message(
                "Finish McSpeechface setup, then open the audio file again."
            ))
            return
        }
        guard urls.count == 1, let url = urls.first else {
            presentError(McSpeechfaceError.message("Open one audio file at a time."))
            return
        }
    ...
    private var currentReleaseTag: String? {
        Bundle.main.object(forInfoDictionaryKey: "McSpeechfaceReleaseTag") as? String
    }
