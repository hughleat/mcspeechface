...
enum AppPaths {
    ...
    private static let rawApplicationSupportDirectory: URL = {
        if let configuredPath = ProcessInfo.processInfo.environment["MCSPEECHFACE_DATA_DIR"] {
            return URL(fileURLWithPath: configuredPath, isDirectory: true)
        }
        return fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("McSpeechface", isDirectory: true)
    }()

    /// The checkout root used only by development builds and legacy migration.
    static let projectRoot: URL = {
        if let configuredPath = ProcessInfo.processInfo.environment["MCSPEECHFACE_PROJECT_ROOT"] {
    ...
    /// Evaluating a mutable path attempts each supported migration once per process.
    static let migrationResult: Result<MigrationReport, Error> = Result {
        if ProcessInfo.processInfo.environment["MCSPEECHFACE_DATA_DIR"] == nil {
            let report = try LegacyInstallationMigrator.migrateApplicationSupportIfNeeded(
                currentDirectory: rawApplicationSupportDirectory
            )
            if !report.skippedItems.isEmpty {
                throw LegacyInstallationMigrator.MigrationError(
                    detail: "Some earlier items conflict with existing McSpeechface data and were left in place: \(report.skippedItems.joined(separator: ", "))"
                )
            }
        }
        return try migrateLegacyProjectDataIfNeeded()
    }
    ...
    static var llamaServerHelperExecutable: URL {
        if let configuredPath = ProcessInfo.processInfo.environment[
            "MCSPEECHFACE_LLAMA_SERVER_EXECUTABLE"
        ] {
            return URL(fileURLWithPath: configuredPath)
        }
        return Bundle.main.bundleURL
            .appendingPathComponent("Contents/Helpers/llama/mcspeechface-llama-server")
    }
    ...
    private static func isProjectRoot(_ url: URL) -> Bool {
        fileManager.fileExists(atPath: url.appendingPathComponent("Package.swift").path)
            && fileManager.fileExists(
                atPath: url.appendingPathComponent("Sources/McSpeechface/AppDelegate.swift").path
            )
    }
