...
final class HistoryView: NSStackView, NSSearchFieldDelegate, NSTableViewDataSource,
    ...
    init(service: McSpeechfaceService) {
        ...
        audioTask = Task { [weak self] in
            ...
            do {
                ...
                guard player.prepareToPlay(), player.play() else {
                    throw McSpeechfaceError.message("The recording could not be played.")
                }
                audioPlayer = player
            } catch {
                guard !Task.isCancelled else { return }
    ...
    nonisolated func audioPlayerDecodeErrorDidOccur(_ player: AVAudioPlayer, error: Error?) {
        Task { @MainActor [weak self] in
            guard let self, audioPlayer === player else { return }
            stopPlayback()
            window?.presentError(error ?? McSpeechfaceError.message("The recording could not be decoded."))
        }
    }
    ...
    private static func parseDate(_ value: String) -> Date? {
        ISO8601DateFormatter.mcspeechfaceWithFractionalSeconds.date(from: value)
            ?? ISO8601DateFormatter.mcspeechfaceWithoutFractionalSeconds.date(from: value)
    }
...
private extension ISO8601DateFormatter {
    static let mcspeechfaceWithFractionalSeconds: ISO8601DateFormatter = {
        let formatter = ISO8601DateFormatter()
        formatter.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
        return formatter
    }()

    static let mcspeechfaceWithoutFractionalSeconds: ISO8601DateFormatter = {
        let formatter = ISO8601DateFormatter()
