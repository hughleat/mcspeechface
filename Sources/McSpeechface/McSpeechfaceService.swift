...
extension Notification.Name {
    static let mcspeechfaceModelUseDidChange = Notification.Name("McSpeechfaceModelUseDidChange")
}

@MainActor
final class McSpeechfaceService {
    ...
    private let store: NativeMcSpeechfaceStore?
    ...
    func beginRecordingModelUse() throws {
        guard !modelOperationInProgress, !modelUseInProgress else {
            throw McSpeechfaceError.message("Wait for the current model operation to finish.")
        }
    ...
    init(
        ...
        do {
            store = try NativeMcSpeechfaceStore(rootURL: AppPaths.dataDirectory)
            storeError = nil
        } catch {
            store = nil
            storeError = error
    ...
    private func performTranscription(
        ...
        if identifySpeakers {
            guard model.supportsSpeakerIdentification else {
                throw McSpeechfaceError.message(
    ...
    func preload(
        ...
        if model.key == DictationModel.appleSpeechKey {
            ...
            throw McSpeechfaceError.message("The selected transcription model is unavailable.")
        }
    }
    ...
    private func postModelUseChange() {
        NotificationCenter.default.post(name: .mcspeechfaceModelUseDidChange, object: nil)
    }
    ...
    nonisolated static func validatePrivacySettings(_ settings: PrivacySettings) throws {
        ...
        guard NativePrivacySettings.allowedRetentionDays.contains(settings.retention_days) else {
            throw McSpeechfaceError.message("Retention must be Forever, 1, 7, 30, or 90 days.")
        }
    }
    ...
    private func requireStore() throws -> NativeMcSpeechfaceStore {
        if let store { return store }
        throw storeError ?? McSpeechfaceError.message("McSpeechface could not open its local data store.")
    }
    ...
    nonisolated static func requireDetectedSpeech(in text: String) throws {
        guard !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            throw McSpeechfaceError.noSpeechDetected
        }
    }
...
enum McSpeechfaceError: LocalizedError {
