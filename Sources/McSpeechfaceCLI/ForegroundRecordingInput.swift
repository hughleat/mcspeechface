...
enum ForegroundRecordingLeaseSnapshot {
    case pending
    case started(String)
    case completed(McSpeechfaceCommandMessage)
    case failed(Error)
}
...
final class ForegroundRecordingLeaseState: @unchecked Sendable {
    ...
    private var completion: Result<McSpeechfaceCommandMessage, Error>?

    func receive(_ event: McSpeechfaceCommandEvent) {
    ...
    func complete(_ result: Result<McSpeechfaceCommandMessage, Error>) {
        lock.withLock {
            completion = result
        }
    }
