...
enum McSpeechfaceCLI {
    static func run() -> Int32 {
        ...
        case .version:
            let appURL = McSpeechfaceAppLocator.appURL()
            write(Data((McSpeechfaceAppLocator.version(appURL: appURL) + "\n").utf8), to: .standardOutput)
            return CLIExitCode.success.rawValue
    ...
    private static func makeRequest(_ invocation: CLIInvocation) throws -> McSpeechfaceCommandRequest {
        switch invocation {
        case .status:
            return .status()
        case .models:
    ...
    private static func writeResponse(
        _ response: McSpeechfaceCommandMessage,
        format: CLIOutputFormat
    ) throws -> Int32 {
    ...
    private static func sendLaunchingAppIfNeeded(
        ...
        onEvent: McSpeechfaceCommandSocketClient.EventHandler? = nil
    ) throws -> McSpeechfaceCommandMessage {
        let socketURL = McSpeechfaceCommandSocketPath.defaultURL()
        let client = McSpeechfaceCommandSocketClient(socketURL: socketURL)
        do {
            return try client.send(request, onEvent: onEvent)
        } catch let error as McSpeechfaceSocketError where error.isRetryableConnectionFailure {
            guard let appURL = McSpeechfaceAppLocator.appURL() else {
                throw CLIExecutionError.appNotFound
            }
            try McSpeechfaceAppLauncher.launch(appURL)
            ...
            while Date() < deadline {
                ...
                do {
                    return try client.send(request, onEvent: onEvent)
                } catch let retry as McSpeechfaceSocketError where retry.isRetryableConnectionFailure {
                    lastError = retry
                }
            }
            throw lastError
    ...
    private static func exitCode(for error: Error) -> CLIExitCode {
        if let socketError = error as? McSpeechfaceSocketError {
            switch socketError {
    ...
    private static func errorCode(for error: Error) -> String {
        ...
        case CLIExecutionError.standardInputFailed:
            "input_error"
        case let socket as McSpeechfaceSocketError where socket == .timedOut:
            "timeout"
        case let socket as McSpeechfaceSocketError:
            if case .peerUIDMismatch = socket { "permission_denied" } else { "transport_error" }
        case is McSpeechfaceProtocolError:
            "protocol_error"
        default:
            "internal_error"
        }
    ...
    private static func writeFailure(
        ...
        do {
            ...
            write(Data(("mcspeechface: \(message)\n").utf8), to: .standardError)
        }
    }
...
private extension CLICorrectionOptions {
    var request: McSpeechfaceCommandCorrection? {
        enabled ? McSpeechfaceCommandCorrection(model: model, instructions: instructions) : nil
    }
}

Darwin.exit(McSpeechfaceCLI.run())
