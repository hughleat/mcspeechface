...
public final class McSpeechfaceCommandSocketServer: @unchecked Sendable {
    public typealias Handler = @Sendable (
        McSpeechfaceCommandRequest,
        McSpeechfaceCommandResponder
    ) async throws -> Void
    ...
    private let acceptQueue = DispatchQueue(
        label: "com.hughleat.mcspeechface.command-socket.accept",
        qos: .utility
    )
    ...
    public func start(handler: @escaping Handler) throws {
        ...
        guard requestTimeout.isFinite, requestTimeout > 0, requestTimeout <= 60 else {
            throw McSpeechfaceCommandSocketServerError.invalidRequestTimeout
        }

        let descriptor: Int32 = try stateLock.withLock {
            guard listener < 0 else {
                throw McSpeechfaceCommandSocketServerError.alreadyRunning
            }
    ...
    private func receiveRequest(from socket: ManagedSocket) {
        do {
            try McSpeechfaceCommandSocketSecurity.validatePeer(socket.descriptor)
            let data = try Self.readRequest(
                from: socket,
                timeout: requestTimeout
            )
            let request: McSpeechfaceCommandRequest
            do {
                request = try JSONDecoder().decode(McSpeechfaceCommandRequest.self, from: data)
                _ = try request.validated()
            } catch let error as McSpeechfaceProtocolError {
                throw error
            } catch {
            ...
            let responder = McSpeechfaceCommandResponder(
                request: request,
                socket: socket,
    ...
    private static func readRequest(
        ...
        while true {
            ...
            guard remaining > 0 else {
                throw McSpeechfaceCommandSocketServerError.requestTimedOut
            }
            ...
            guard pending.count <= McSpeechfaceProtocolLimits.maximumRequestBytes + 1 else {
                throw McSpeechfaceProtocolError.requestTooLarge
            }
            if let newline = pending.firstIndex(of: 0x0A) {
                let line = pending.prefix(upTo: newline)
                guard !line.isEmpty,
                      line.count <= McSpeechfaceProtocolLimits.maximumRequestBytes else {
                    throw McSpeechfaceProtocolError.invalidRequest(
    ...
    private static func preparePrivateDirectory(_ url: URL) throws {
        ...
        if mkdir(path, 0o700) == 0 {
            ...
            throw McSpeechfaceSocketError.systemCall("mkdir", errno)
        }
        if created, chmod(path, 0o700) != 0 {
            throw McSpeechfaceSocketError.systemCall("chmod", errno)
        }
        let directoryIdentity = try identity(at: url, expectedType: S_IFDIR)
        guard directoryIdentity.owner == geteuid() else {
            throw McSpeechfaceCommandSocketServerError.unsafeParentDirectory
        }
    ...
    private static func acquireServerLock(for socketURL: URL) throws -> Int32 {
        ...
        guard descriptor >= 0 else {
            throw McSpeechfaceSocketError.systemCall("open", errno)
        }
        do {
            var status = stat()
            guard fstat(descriptor, &status) == 0 else {
                throw McSpeechfaceSocketError.systemCall("fstat", errno)
            }
            guard status.st_uid == geteuid(),
                  status.st_mode & S_IFMT == S_IFREG else {
                throw McSpeechfaceCommandSocketServerError.unsafeLockFile
            }
            guard fchmod(descriptor, S_IRUSR | S_IWUSR) == 0 else {
                throw McSpeechfaceSocketError.systemCall("fchmod", errno)
            }
            guard flock(descriptor, LOCK_EX | LOCK_NB) == 0 else {
                if errno == EWOULDBLOCK {
                    throw McSpeechfaceCommandSocketServerError.alreadyRunning
                }
                throw McSpeechfaceSocketError.systemCall("flock", errno)
            }
            return descriptor
        } catch {
            close(descriptor)
    ...
    private static func removeStaleSocketIfSafe(at url: URL) throws {
        ...
        guard lstat(url.path, &status) == 0 else {
            ...
            throw McSpeechfaceSocketError.systemCall("lstat", errno)
        }
        guard status.st_uid == geteuid(),
              status.st_mode & S_IFMT == S_IFSOCK else {
            throw McSpeechfaceCommandSocketServerError.unsafeExistingSocketPath
        }
        guard unlink(url.path) == 0 else {
            throw McSpeechfaceSocketError.systemCall("unlink", errno)
        }
    }
    ...
    private static func makeListener(at url: URL) throws -> Int32 {
        ...
        do {
            ...
            guard result == 0 else {
                throw McSpeechfaceSocketError.systemCall("bind", errno)
            }
            guard chmod(url.path, 0o600) == 0 else {
                throw McSpeechfaceSocketError.systemCall("chmod", errno)
            }
            guard listen(descriptor, 16) == 0 else {
                throw McSpeechfaceSocketError.systemCall("listen", errno)
            }
            return descriptor
        } catch {
            close(descriptor)
    ...
    private static func identity(
        ...
        guard status.st_mode & S_IFMT == expectedType else {
            throw McSpeechfaceCommandSocketServerError.unexpectedFileType
        }
        return FileIdentity(
...
public actor McSpeechfaceCommandResponder {
    private let request: McSpeechfaceCommandRequest
    ...
    public func sendEvent(
        ...
        guard !terminalSent else {
            throw McSpeechfaceCommandSocketServerError.terminalAlreadySent
        }
        try send(.event(
            id: request.id,
            name: name,
    ...
    public func sendSuccess(_ result: McSpeechfaceCommandResult) throws {
    ...
    private func sendTerminal(_ message: McSpeechfaceCommandMessage) throws {
        guard !terminalSent else {
            throw McSpeechfaceCommandSocketServerError.terminalAlreadySent
        }
        terminalSent = true
    ...
    private func send(_ message: McSpeechfaceCommandMessage) throws {
        ...
        data.append(0x0A)
        guard data.count <= McSpeechfaceProtocolLimits.maximumMessageBytes else {
            throw McSpeechfaceProtocolError.messageTooLarge
        }
        responseBytes += data.count
        guard responseBytes <= McSpeechfaceProtocolLimits.maximumResponseBytes else {
...
public enum McSpeechfaceCommandSocketServerError: Error, Equatable, LocalizedError {
    case alreadyRunning
...
private final class ManagedSocket: @unchecked Sendable {
    ...
    func duplicateDescriptor() throws -> Int32 {
        try lock.withLock {
            guard !closed else {
                throw McSpeechfaceCommandSocketServerError.connectionClosed
            }
            let duplicate = dup(descriptor)
            guard duplicate >= 0 else {
                throw McSpeechfaceSocketError.systemCall("dup", errno)
            }
            return duplicate
        }
    }
    ...
    func write(_ data: Data, timeout: TimeInterval) throws {
        ...
        try data.withUnsafeBytes { bytes in
            ...
            while offset < bytes.count {
                let remaining = deadline.timeIntervalSinceNow
                guard remaining > 0 else {
                    throw McSpeechfaceSocketError.timedOut
                }
                var pollDescriptor = pollfd(
                    fd: writeDescriptor,
