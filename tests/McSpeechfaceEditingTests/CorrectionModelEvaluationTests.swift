...
struct CorrectionModelEvaluationTests {
    ...
    func evaluateInstalledModelsWhenRequested() async throws {
        ...
        guard let requestedModels = environment["MCSPEECHFACE_CORRECTION_EVALUATION_MODELS"] else {
            return
        }
        let assertExact = environment["MCSPEECHFACE_CORRECTION_EVALUATION_ASSERT_EXACT"] == "1"

        if requestedModels.contains("qwen") {
            guard let executablePath = environment["MCSPEECHFACE_LLAMA_SERVER_EXECUTABLE"],
                  let modelPath = environment["MCSPEECHFACE_QWEN_MODEL"] else {
    ...
    private func write(_ result: EvaluationResult) {
        ...
        FileHandle.standardError.write(
            Data(("MCSPEECHFACE_CORRECTION_EVAL " + String(decoding: data, as: UTF8.self) + "\n").utf8)
        )
    }
}
