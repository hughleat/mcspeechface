...
import Foundation
import Testing
@testable import McSpeechfaceCLI
...
struct CLIArgumentsTests {
    ...
    func rejectsInvalidCorrectionOptions() {
        ...
        #expect(throws: CLIArgumentError.self) {
            try CLIArguments.parse([
                ...
                String(repeating: "x", count: McSpeechfaceProtocolLimits.maximumInstructionsBytes + 1),
            ])
        }
    }
    ...
    func foregroundLeasePublishesItsSessionAndCompletion() throws {
        ...
        lease.receive(McSpeechfaceCommandEvent(name: "recording", detail: session))
        ...
        let request = McSpeechfaceCommandRequest.recordStart(
            model: nil,
            saveHistory: true,
            lease: true
        )
        lease.complete(.success(.success(
            id: request.id,
            result: McSpeechfaceCommandResult(kind: "lease_released", state: "idle")
        )))
    ...
    func outputKeepsTextAndJSONMachineReadable() throws {
        ...
        let message = McSpeechfaceCommandMessage.success(
            id: request.id,
        ...
        let correctionModels = McSpeechfaceCommandMessage.success(
            id: request.id,
            result: McSpeechfaceCommandResult(
                kind: "correction_models",
                correctionModels: [
                    McSpeechfaceCommandCorrectionModel(
                        key: "codex",
                        name: "Codex",
                        available: true,
                        selected: true,
