...
struct CommandSocketServerTests {
    ...
    func serverCreatesPrivatePathsAndCleansUp() throws {
        ...
        let server = McSpeechfaceCommandSocketServer(socketURL: fixture.socketURL)

        try server.start { _, responder in
            try await responder.sendSuccess(
                McSpeechfaceCommandResult(kind: "status", state: "idle")
            )
        }
    ...
    func serverNeverChangesPermissionsOfExistingDirectory() throws {
        ...
        let root = URL(
            fileURLWithPath: "/tmp/mcspeechface-parent-\(identifier)",
            isDirectory: true
        )
        let directory = root.appendingPathComponent("McSpeechface", isDirectory: true)
        ...
        let server = McSpeechfaceCommandSocketServer(
            socketURL: directory.appendingPathComponent("command.sock")
        )
        #expect(throws: McSpeechfaceCommandSocketServerError.unsafeParentDirectory) {
    ...
    func serverSendsEventsAndOneTerminalResponse() throws {
        ...
        try server.start { _, responder in
            ...
            try await responder.sendSuccess(
                McSpeechfaceCommandResult(kind: "status", state: "ready")
            )
            do {
            ...
            } catch McSpeechfaceCommandSocketServerError.terminalAlreadySent {
                terminalCheck.set(true)
            }
        }

        let events = LockedValue<[McSpeechfaceCommandEvent]>([])
        let response = try McSpeechfaceCommandSocketClient(
    ...
    func serverNeverRemovesNonSocketAtDestination() throws {
        ...
        #expect(throws: McSpeechfaceCommandSocketServerError.self) {
            try server.start { _, _ in }
        }
        #expect(
    ...
    func secondServerCannotReplaceLiveServer() throws {
        let fixture = try ServerFixture()
        defer { fixture.remove() }
        let first = McSpeechfaceCommandSocketServer(socketURL: fixture.socketURL)
        let second = McSpeechfaceCommandSocketServer(socketURL: fixture.socketURL)
        defer {
            first.stop()
            second.stop()
        }
...
private struct ServerFixture {
    ...
    init() throws {
        let identifier = UUID().uuidString.prefix(12)
        let root = URL(
            fileURLWithPath: "/tmp/mcspeechface-server-\(identifier)",
            isDirectory: true
        )
        directory = root.appendingPathComponent("McSpeechface", isDirectory: true)
        socketURL = directory.appendingPathComponent("command.sock")
...
private func connectWithoutReading(to url: URL) throws -> Int32 {
    ...
    do {
        ...
        guard result == 0 else {
            throw McSpeechfaceSocketError.systemCall("connect", errno)
        }
        return descriptor
    } catch {
...
private func sendStatusRequest(to descriptor: Int32) throws {
    var data = try JSONEncoder().encode(McSpeechfaceCommandRequest.status())
    data.append(0x0A)
