...
struct CommandSocketTests {
    ...
    func clientRejectsMismatchedResponseID() throws {
        ...
        #expect(throws: McSpeechfaceProtocolError.self) {
            try McSpeechfaceCommandSocketClient(socketURL: server.url, timeout: 10)
                .send(.status())
        }
    }
    ...
    func clientRejectsOversizedMessage() throws {
        let server = try TestSocketServer(rawResponse: Data(
            ...
            count: McSpeechfaceProtocolLimits.maximumMessageBytes + 1
        ))
        defer { server.close() }
    ...
    func peerValidationAcceptsCurrentUser() throws {
        ...
        try McSpeechfaceCommandSocketSecurity.validatePeer(descriptors[0])
        #expect(throws: McSpeechfaceSocketError.self) {
            try McSpeechfaceCommandSocketSecurity.validatePeer(
                descriptors[0],
...
private final class EventRecorder: @unchecked Sendable {
    ...
    private var storage: [McSpeechfaceCommandEvent] = []

    var values: [McSpeechfaceCommandEvent] {
        lock.withLock { storage }
    }

    func append(_ event: McSpeechfaceCommandEvent) {
...
private final class TestSocketServer: @unchecked Sendable {
    ...
    private let lock = NSLock()
    private var storedRequest: McSpeechfaceCommandRequest?

    init(
        responses: @escaping @Sendable (McSpeechfaceCommandRequest) -> [McSpeechfaceCommandMessage]
    ) throws {
    ...
    func receivedRequest() throws -> McSpeechfaceCommandRequest? {
    ...
    private func serve(response: (McSpeechfaceCommandRequest) -> Data) {
        ...
        try? McSpeechfaceCommandSocketSecurity.validatePeer(client)
    ...
    private static func makeListener(url: URL) throws -> Int32 {
        try McSpeechfaceCommandSocketPath.validate(url)
    ...
    private static func makeSocketURL() throws -> URL {
        ...
        let root = URL(
            fileURLWithPath: "/tmp/mcspeechface-ipc-\(identifier)",
            isDirectory: true
        )
