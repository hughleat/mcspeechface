...
struct CoreMLParakeetEngineTests {
    ...
    func testDerivesCanonicalDirectoryFromConfiguredRoot() {
        let engine = CoreMLParakeetEngine(
            modelsRootDirectory: URL(fileURLWithPath: "/tmp/mcspeechface-coreml")
        )

        #expect(engine.modelsRootDirectory.path == "/tmp/mcspeechface-coreml")
        #expect(
            engine.modelDirectory.path
                == "/tmp/mcspeechface-coreml/parakeet-tdt-ctc-110m"
        )
    }
    ...
    func testInstalledModelTranscribesRealAudioWhenConfigured() async throws {
        ...
        guard
            let rootPath = environment["MCSPEECHFACE_COREML_TEST_MODEL_ROOT"],
            let audioPath = environment["MCSPEECHFACE_COREML_TEST_AUDIO"]
        else {
            return
        }
        let engine = CoreMLParakeetEngine(
        ...
        if environment["MCSPEECHFACE_COREML_TEST_DOWNLOAD"] == "1",
    ...
    func unifiedModelTranscribesRealAudioWhenExplicitlyConfigured() async throws {
        let environment = ProcessInfo.processInfo.environment
        guard
            environment["MCSPEECHFACE_UNIFIED_COREML_TEST"] == "1",
            let rootPath = environment["MCSPEECHFACE_COREML_TEST_MODEL_ROOT"],
            let audioPath = environment["MCSPEECHFACE_COREML_TEST_AUDIO"]
        else {
            return
