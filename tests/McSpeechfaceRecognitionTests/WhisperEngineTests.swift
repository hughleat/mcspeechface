...
struct WhisperEngineTests {
    ...
    func derivesCanonicalDirectoryFromConfiguredRoot() {
        let engine = CoreMLWhisperEngine(
            model: .small,
            modelsRootDirectory: URL(fileURLWithPath: "/tmp/mcspeechface-whisper")
        )

        #expect(engine.modelsRootDirectory.path == "/tmp/mcspeechface-whisper")
        #expect(
            engine.modelDirectory.path
                == "/tmp/mcspeechface-whisper/openai_whisper-small"
        )
    }
