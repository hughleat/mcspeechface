...
struct ErrorRecoveryTests {
    ...
    func knownErrorsMapToRecoveryActions() {
        ...
        #expect(
            ErrorRecovery.presentation(
                for: McSpeechfaceError.message("Speech Recognition permission is required.")
            ).action == .openSpeechRecognitionSettings
        )
        #expect(
            ErrorRecovery.presentation(
                for: McSpeechfaceError.message("On-device Apple Speech is unavailable.")
            ).action == .openModels
        )
        #expect(ErrorRecovery.presentation(for: McSpeechfaceError.message("Model is not installed.")).action == .openModels)
        #expect(ErrorRecovery.presentation(for: McSpeechfaceError.message("Could not decode audio.")).action == .retryTranscription)
        #expect(McSpeechfaceError.noSpeechDetected.errorDescription == "No speech was detected.")
    ...
    func emptyFinalTranscriptIsRejectedAsNoSpeech() {
        do {
            try McSpeechfaceService.requireDetectedSpeech(in: " \n\t ")
        ...
        do {
            try McSpeechfaceService.requireDetectedSpeech(in: "spoken words")
            try McSpeechfaceService.requireDetectedSpeech(in: "new line")
        } catch {
            Issue.record("Unexpected error: \(error)")
        }
    }
