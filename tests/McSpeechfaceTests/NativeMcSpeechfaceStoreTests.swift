...
@Suite(.serialized)
struct NativeMcSpeechfaceStoreTests {
    @Test
    ...
    func audioReferencesCannotEscapeThroughSymlinkedAncestors() async throws {
        try await withStore { store, root in
            let outside = root.deletingLastPathComponent()
                .appendingPathComponent("McSpeechfaceOutside-\(UUID().uuidString)", isDirectory: true)
    ...
    private func withStore(
        _ body: (NativeMcSpeechfaceStore, URL) async throws -> Void
    ) async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("McSpeechfaceNativeTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let store = try NativeMcSpeechfaceStore(rootURL: root)
