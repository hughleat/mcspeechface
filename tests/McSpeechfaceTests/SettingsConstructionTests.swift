...
struct SettingsConstructionTests {
    ...
    func settingsWindowCanBeConstructedDuringLaunch() throws {
        _ = NSApplication.shared
        let controller = SettingsWindowController(service: McSpeechfaceService())
    ...
    func correctionExecutionSnapshotDoesNotFollowLaterSettingsChanges() async throws {
        ...
        defer { defaults.removePersistentDomain(forName: suite) }
        let service = TranscriptEditingService(defaults: defaults)
        let firstCommand = McSpeechface.CommandLineCorrectionConfiguration(
            preset: .custom,
            executablePath: "/usr/bin/true",
            model: "first",
            arguments: []
        ...
        let secondCommand = McSpeechface.CommandLineCorrectionConfiguration(
            preset: .custom,
    ...
    func correctionPromptPreferencesRoundTripAndReset() throws {
        let suiteName = "mcspeechface-prompt-tests-\(UUID().uuidString)"
    ...
    func invalidStoredCorrectionPromptFallsBackToDefaults() throws {
        let suiteName = "mcspeechface-invalid-prompt-tests-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
    ...
    func correctionPromptPreferencesRejectPromptsThatCrowdOutTranscripts() throws {
        let suiteName = "mcspeechface-oversized-prompt-tests-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
        let preferences = TranscriptEditingPromptPreferences(defaults: defaults)
    ...
    func correctionRequestReceivesSelectedLanguageAndOmitsAutomaticLanguage() {
        let english = transcriptionResponse(language: McSpeechfaceService.finalizationLanguage(for: .english))
        let automatic = transcriptionResponse(language: McSpeechfaceService.finalizationLanguage(for: .auto))
    ...
    func correctionPromptEditorCanResetSavedOverrides() throws {
        _ = NSApplication.shared
        let suiteName = "mcspeechface-prompt-editor-tests-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
        let preferences = TranscriptEditingPromptPreferences(defaults: defaults)
    ...
    func correctionPromptPreferencesLoadLegacyCustomPrompts() throws {
        let suiteName = "mcspeechface-legacy-prompt-tests-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
    ...
    func correctionInstructionsAreVisibleAsASetting() throws {
        _ = NSApplication.shared
        let suiteName = "mcspeechface-correction-selection-tests-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
    ...
    func historyRowActionsStayAtTheTrailingEdge() throws {
        _ = NSApplication.shared
        let target = HistoryView(service: McSpeechfaceService())
        let entries = [
    ...
    @Test
    func commandLineInstallerRecognizesOnlyValidatedMcSpeechfaceLinks() throws {
        let fixture = try CommandLineInstallerFixture()
        defer { fixture.remove() }
        ...
        try FileManager.default.removeItem(at: fixture.linkURL)
        let oldHelper = try fixture.makeApp(in: "old", bundleIdentifier: "com.hughleat.mcspeechface")
        try FileManager.default.createSymbolicLink(
            at: fixture.linkURL,
...
private struct CommandLineInstallerFixture {
    ...
    init() throws {
        root = FileManager.default.temporaryDirectory
            .appendingPathComponent("mcspeechface-installer-tests-\(UUID().uuidString)", isDirectory: true)
        bundleURL = root.appendingPathComponent("McSpeechface.app", isDirectory: true)
        linkURL = root.appendingPathComponent("bin/mcspeechface")
        ...
        helperURL = try Self.makeApp(
            at: bundleURL,
            bundleIdentifier: "com.hughleat.mcspeechface"
        )
    }
    ...
    func makeApp(in directory: String, bundleIdentifier: String) throws -> URL {
        try Self.makeApp(
            at: root
                ...
                .appendingPathComponent("McSpeechface.app", isDirectory: true),
